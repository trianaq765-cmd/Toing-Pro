...
return i.editReply(`✅ **Test Success!**\n📊 Original: ${testScript.length}B\n📊 Output: ${result.code.length}B (${ratio}%)\n🔍 Preview:\n\`\`\`lua\n${result.code.substring(0,300)}...\n\`\`\``);
}
...
return i.editReply({embeds:[new EmbedBuilder().setTitle('📊 Status').setColor(0x2ecc71).addFields({name:'Bot',value:'✅ Online',inline:true},{name:'jnkie API',value:JNKIE_API_KEY?'✅ Connected':'❌ Not Set',inline:true},{name:'LuaFree API',value:LUAFREE_API_KEY?'✅ Connected':'❌ Not Set',inline:true},{name:'Service ID',value:SERVICE_ID,inline:true},{name:'Provider ID',value:PROVIDER_ID,inline:true})]});
}

// ===== HELP =====
if(cmd==='help'){
return i.reply({embeds:[new EmbedBuilder().setTitle('📖 Help').setColor(0x5865F2).addFields({name:'🔒 Obfuscate',value:'`/obf` Prometheus Obfuscator\n`/lua` Lua Free Custom\n`/luapreset` Lua Free Preset\n`/testlua` Debug API'},{name:'🔑 Keys',value:'`/key list` List keys\n`/key create` Create key\n`/key batch` Batch create\n`/key delete` Delete key\n`/key reset` Reset HWID'},{name:'⚙️ Management',value:'`/service` Service Management\n`/provider` Provider Management\n`/integration` Integration'})]});
}
}
...
if(i.customId==='status'){await i.deferUpdate();return i.editReply({embeds:[new EmbedBuilder().setTitle('📊 Status').setColor(0x2ecc71).addFields({name:'Bot',value:'✅',inline:true},{name:'jnkie',value:JNKIE_API_KEY?'✅':'❌',inline:true},{name:'LuaFree',value:LUAFREE_API_KEY?'✅':'❌',inline:true})],components:[]});}
await i.deferUpdate();
}
